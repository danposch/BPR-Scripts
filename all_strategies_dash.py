...
RUNS = 25
STRATEGIES=["best-route", "ncc", "saf", "omccrf", "broadcast" ]
#STRATEGIES=["best-route", "ncc"]

ADAPTATIONLOGICS = {}
ADAPTATIONLOGICS["SVCRateBasedAdaptationLogic"] = "player::SVCRateBasedAdaptationLogic"
ADAPTATIONLOGICS["SVCBufferAdaptationLogic"] = "player::SVCBufferAdaptationLogic"

BW = {}
#BW["LowBW"] = [2000,3000]
BW["MediumBW"] = [3000,4000]
#BW["HighBW"] = [4000,5000]
#BW["MediumBW"] = [6000,8000]
...
def setAdaptationLogic(logic):
	...
	for line in old_f:
		if "CONSUMER_ADAPTATION_LOGIC =" in line:
			new_f.write("CONSUMER_ADAPTATION_LOGIC = \"" + logic +"\"\n")
		else:
			new_f.write(line)
...
def setForwardingStrategy(strategy):
	...
	for line in old_f:
		if "FORWARDING_STRATEGY =" in line:
			new_f.write("FORWARDING_STRATEGY = \"" + strategy +"\"\n")
		else:
			new_f.write(line)
...
#main
different_settings = len(STRATEGIES)*len(BW)*len(CONNECTIVITY)*len(ADAPTATIONLOGICS)
print "Performing " + str(different_settings) + " Settings a " + str(RUNS) + " runs (" + str(different_settings*RUNS) + " total runs)."
print "AdaptationLogics:" + str(ADAPTATIONLOGICS)
...
for s in STRATEGIES:
	...
	for logic in ADAPTATIONLOGICS:
	
		print "Setting Adaptation Logic: " + str(logic) + " (" + str(ADAPTATIONLOGICS[logic]) + ")"
		setAdaptationLogic(ADAPTATIONLOGICS[logic])
		...
		for bw in BW:
			...
			for con in CONNECTIVITY: 
				...
				sysCall = ["python"] + ["emulation.py"] + ["--fw-strategies="+str(s)] +	["--emulation-runs=" + str(RUNS)] +	["--result-folder=" + os.getenv("HOME") + "/emulation_results/" + s + bw +con + "_"+logic]
